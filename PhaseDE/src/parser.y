...
expr:   assignexpr              {
        ...
        | expr EQUAL expr       {
                                        ...
                                        if ($1->type != boolexpr_e) {
                                                convertToBool($1);
                                                if (camefromnot) {
                                                        std::vector<unsigned int> temp = $1->truelist;
...
elist: expr                     {
        ...
        | elist COMMA expr      {       
                                        if ($3->type == boolexpr_e) {
                                                convertToBool($3);
                                                if (camefromnot) {
                                                        std::vector<unsigned int> temp = $3->truelist;
                                                        $3->truelist = $1->falselist;
                                                        $3->falselist = temp;
                                                        camefromnot = false;
                                                }
                                                backpatch($3->truelist, nextquad());
...
forprefix: FOR LEFT_PARENTHESIS  elist SEMICOLON M expr SEMICOLON       {       
                                                                                fprintf(yyout, "                         %d", $6->type);
                                                                                if ($6->type != boolexpr_e) {
                                                                                        convertToBool($6);
                                                                                        if (camefromnot) {
                                                                                                std::vector<unsigned int> temp = $6->truelist;
                                                                                                $6->truelist = $6->falselist;
                                                                                                $6->falselist = temp;
                                                                                                camefromnot = false;
                                                                                        }
                                                                                        backpatch($6->truelist, nextquad());
                                                                                        backpatch($6->falselist, nextquad() + 2);
                                                                                        emit(assign, newexpr_constbool(true), nullptr, $3, 0);
                                                                                        emit(jump, nullptr, nullptr, nullptr, nextquad() + 2);
                                                                                        emit(assign, newexpr_constbool(false), nullptr, $3, 0);
